...
- Log a session with automatic device saves, per-set previous results, live PR markers and congratulations banners, a rest countdown, restart recovery, and partial-session completion.
...
Use **Programs → Create program** to build a private custom program with 1–52 weeks and up to seven workout days per week. Each day has its own schedule label, ordered exercise list, sets, reps, rest and effort targets. Programs can be duplicated, paused and safely removed. Custom programs can be edited from Today or My programs. Edits made before recording sessions update the program directly; editing after history exists creates a revised program from week 1 and preserves the original completed sessions. When finishing a custom-program workout, choose whether exercise additions, removals, order and target changes apply only today or replace that workout in future weeks. Custom programs are included in backups and account sync.
